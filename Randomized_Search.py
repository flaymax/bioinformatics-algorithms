...
def Profile(Motifs):
    ...
    for Motif in Motifs:
        for i, nucleotide in enumerate(Motif):
            ...
            profile[nucleotide_position][i] += 1 / (2 * len(Motifs))
    return profile
...
def Randomized_MS(DNA, k, t):
    best_motifs = []
    
    for Pattern in DNA:
        N = random.randint(0, len(DNA[0]) - k)
        best_motifs.append(Pattern[N: N + k])
    best_score = Score(best_motifs)
    current_motifs = best_motifs
    
    while 1:
        curr_profile = Profile(current_motifs)
        #print(curr_profile)
        current_motifs = []
        for Pattern in DNA:
            current_motifs.append(max_prob_kmer(Pattern, curr_profile, k))
        curr_score = Score(current_motifs)
        if curr_score < best_score:
            best_score = curr_score
...
def main_function(DNA, k, t):
    best_motifs = Randomized_MS(DNA, k, t)
    best_score = Score(best_motifs)
    for i in range(500): # надо подбирать
        get_motifs = Randomized_MS(DNA, k, t)
        current_score = Score(get_motifs)
        if current_score < best_score:
            best_score = current_score
            best_motifs = get_motifs
    return best_motifs
